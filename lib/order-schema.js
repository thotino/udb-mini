  ...
  return Promise.try(() => {
    return this.findById(orderId)
      ...
      .then((existingOrder) => {
        ...
        if(orderData.userId) { updateData.$set.userId = orderData.userId; }
        if(orderData.orderDate) { updateData.$set.orderDate = orderData.orderDate; }
        if(orderData.orderStatus) { updateData.$set.orderStatus = orderData.orderStatus; }
        if(orderData.familyPack) { updateData.$set.familyPack = orderData.familyPack; }
        if(orderData.figuresQuantity) { updateData.$set.figuresQuantity = orderData.figuresQuantity; }
        if(orderData.serialNumbersList) { updateData.$set.serialNumbersList = orderData.serialNumbersList; }

        return this.findOneAndUpdate(orderId, updateData, {"new": "true"})
          .exec()
